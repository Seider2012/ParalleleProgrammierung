...
static double buffer1[NR];
static double buffer2[NR];
...
static void psinv(void *or, void *ou, int n1, int n2, int n3,
    ...
    double (*u1)[n2][n1] = (double (*)[n2][n1]) buffer1;
    double (*u2)[n2][n1] = (double (*)[n2][n1]) buffer2;
    ...
    if (n1 != 3 && n2 != 3 && n3 != 3) {
        for (i3 = 0; i3 < mm3 - 1; i3++) {
            for (i2 = 0; i2 < mm2 - 1; i2++) {
                z1[0] = z[i3][i2 + 1][0] + z[i3][i2][0];
                z2[0] = z[i3 + 1][i2][0] + z[i3][i2][0];
                z3[0] = z[i3 + 1][i2 + 1][0] + z[i3 + 1][i2][0] + z1[0];
                for (i1 = 1; i1 < mm1; i1++) {
                    int buffer = i1 - 1;
                    z1[i1] = z[i3][i2 + 1][i1] + z[i3][i2][i1];
                    z2[i1] = z[i3 + 1][i2][i1] + z[i3][i2][i1];
                    z3[i1] = z[i3 + 1][i2 + 1][i1] + z[i3 + 1][i2][i1] + z1[i1];
                    u[2 * i3][2 * i2][2 * buffer] = u[2 * i3][2 * i2][2 * buffer]
                                                    + z[i3][i2][buffer];
                    u[2 * i3][2 * i2][2 * buffer + 1] = u[2 * i3][2 * i2][2 * buffer + 1]
                                                        + 0.5 * (z[i3][i2][buffer + 1] + z[i3][i2][buffer]);
                }
